    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/80 p-4">
      <div className="flex max-h-[85vh] w-full max-w-lg flex-col rounded border border-yellow-700 bg-slate-900 shadow-2xl">
        <div className="shrink-0 border-b border-slate-800 px-5 py-3">
          <h2 className="text-sm font-semibold text-yellow-200">{t.permissionDialog.title}</h2>
        </div>
        <div className="grid min-h-0 flex-1 gap-3 overflow-y-auto p-5 text-sm">
          <div>
            <div className="text-xs uppercase tracking-wide text-slate-500">{t.permissionDialog.agent}</div>
            <div className="mt-1 font-medium">{head.agentName}</div>
          </div>
          <div>
            <div className="text-xs uppercase tracking-wide text-slate-500">{t.permissionDialog.tool}</div>
            <div className="mt-1 break-all font-mono text-cyan-300">{head.toolName}</div>
          </div>
          <div className="min-w-0">
            <div className="text-xs uppercase tracking-wide text-slate-500">{t.permissionDialog.input}</div>
          ...
          {queue.length > 1 ? (
            <p className="text-[11px] text-slate-500">{t.permissionDialog.morePending(queue.length - 1)}</p>
          ) : null}
        </div>
